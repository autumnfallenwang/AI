...
def test_model(model, dataloader, criterion):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, labels) in enumerate(dataloader):
            ...
            soft_out = F.softmax(outputs).cpu().numpy().tolist()
            TH = 0.8
            soft_preds=[]
            for s in soft_out:
                if s[1] > TH:
            ...
            metrics.get_batch_cf_matrix(soft_preds, label_list)
...
if MODE == 'TRAIN':
    if FINETUNE:
        print('| Load Pretrained Model: %s' % (MODEL_TYPE+str(MODEL_DEPTH_OR_VERSION)))
    else:
        print('| Load Empty Model: %s' % (MODEL_TYPE+str(MODEL_DEPTH_OR_VERSION)))
    print()
